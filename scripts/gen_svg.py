...
def _with_lightness(value, target):
    r, g, b = _rgb(value)
    hue, _, sat = colorsys.rgb_to_hls(r, g, b)
    return "%02X%02X%02X" % tuple(
        round(c * 255) for c in colorsys.hls_to_rgb(hue, target, sat)
    )


def _fit_contrast(value, chip, target, lighten):
    if _contrast(value, chip) >= target:
        return value
    steps = range(0, 101) if lighten else range(100, -1, -1)
    for step in steps:
        candidate = _with_lightness(value, step / 100.0)
        if _contrast(candidate, chip) >= target:
            return candidate
    return value


def _orbit_theme_colors(slug, brand):
    light = _fit_contrast(brand, CHIP_LIGHT, CONTRAST_LIGHT, False)
    if slug in ORBIT_DARK_BOOST:
        dark = _with_lightness(brand, DARK_BOOST_L)
    else:
        dark = _fit_contrast(brand, CHIP_DARK, CONTRAST_DARK, True)
    return light, dark


def _orbit_has_icon(slug):
    return slug is not None and slug in ORBIT_ICONS


def _orbit_extent(label, slug, weight):
    box = ICON_BOX[weight]
    if _orbit_has_icon(slug):
        return box + 12.0, box + 12.0
    return 14.0 + 6.6 * len(label), box * 0.78
...
def _orbit_icon_defs():
    ...
    for slug, item in ORBIT_ICONS.items():
        parts.append('<g id="i-%s"><path d="%s"/></g>' % (slug, item[1]))
    parts.append('</defs>')
    return "".join(parts)
...
def _orbit_body(label, slug, weight):
    box = ICON_BOX[weight]
    width, height = _orbit_extent(label, slug, weight)
    if not _orbit_has_icon(slug):
        return (
            ...
            % (-width / 2, -height / 2, width, height, height / 2,
               (8 + weight) * 0.36, 8 + weight, label)
        )
    return (
        '<rect class="chip" x="%.1f" y="%.1f" width="%.1f" height="%.1f" rx="10" '
        'stroke-width="1"/>'
        '<use href="#i-%s" xlink:href="#i-%s" class="ic-%s" '
        'transform="translate(%.1f,%.1f) scale(%.4f)"/>'
        % (-width / 2, -height / 2, width, height, slug, slug, slug,
           -box / 2.0, -box / 2.0, box / 24.0)
    )
